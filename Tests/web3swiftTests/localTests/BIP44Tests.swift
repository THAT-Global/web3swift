...
final class BIP44Tests: XCTestCase {
    func testDeriveWithoutThrowOnWarning() async throws {
        ...
        let childNode = try await rootNode.derive(path: "m/44'/60'/8096'/0/1", throwOnWarning: false, transactionChecker:
            mockTransactionChecker)

        XCTAssertEqual(try XCTUnwrap(childNode).publicKey.toHexString(),
                       "035785d4918449c87892371c0f9ccf6e4eda40a7fb0f773f1254c064d3bba64026")
    ...
    func testZeroAccountNeverThrow() async throws {
        ...
        let childNode = try await rootNode.derive(
            path: "m/44'/60'/0'/0/255",
            throwOnWarning: true,
            transactionChecker: mockTransactionChecker
        )

        XCTAssertEqual(
            try XCTUnwrap(childNode).publicKey.toHexString(),
            "0262fba1af8f149258123265318114066decf50d16c1222a9d657b7de2296c2734"
        )
        XCTAssertEqual(mockTransactionChecker.addresses.count, 0)
    }
    ...
    func testFirstAccountWithPreviousTransactionHistory() async throws {
        do {
            ...
            XCTAssertEqual(
                try XCTUnwrap(childNode).publicKey.toHexString(),
                "036cd8f1bad46fa7caf7a80d48528b90db2a3b7a5c9a18d74d61b286e03850abf4"
            )
            XCTAssertEqual(mockTransactionChecker.addresses, accountZeroScannedAddresses)
        } catch BIP44Error.warning {
    ...
    func testNotZeroChangeAndAddressIndexWithPreviousTransactionHistory() async throws {
        do {
            ...
            XCTAssertEqual(
                try XCTUnwrap(childNode).publicKey.toHexString(),
                "0282134e44d4c040a4b4c1a780d8302955096cf1d5e738b161c83f0ce1b863c14e"
            )
            XCTAssertEqual(mockTransactionChecker.addresses, accountZeroScannedAddresses)
        } catch BIP44Error.warning {
            XCTFail("BIP44Error.warning must not be thrown")
...
final class BIP44ErrorTests: XCTestCase {
    func testLocalizedDescription() {
        ...
        XCTAssertEqual(
            ...
            "Couldn't derive key as it doesn't have a previous account with at least one transaction"
        )
    }
}
