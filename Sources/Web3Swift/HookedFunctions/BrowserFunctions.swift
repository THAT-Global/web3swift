...
public final class BrowserFunctions: Sendable {
    ...
    public func getAccounts() async -> [String]? {
        do {
            let accounts: [EthereumAddress] = if let addresses = provider.attachedKeystoreManager?.addresses { addresses } else {
                try await APIRequest.sendRequest(with: provider, for: .getAccounts).result
            }
    ...
    public func sign(_ personalMessage: Data, account: String, password: String ) -> String? {
        do {
            guard let keystoreManager = self.provider.attachedKeystoreManager else { return nil }
    ...
    public func estimateGas(_ transaction: CodableTransaction) async -> BigUInt? {
        do {
            let request = APIRequest.estimateGas(transaction, .latest)
