...
extension Web3 {
    ...
    public struct EIP681CodeParser {
        ...
        static let addressRegex = "^(pay-)?([0-9a-zA-Z.]+)(@[0-9]+)?\\/?(.*)?$"
        
        public static func parse(_ data: Data) async throws -> EIP681Code? {
            guard let string = String(data: data, encoding: .utf8) else { return nil }
            return try await parse(string)
        }
        ...
        public static func parse(_ string: String) async throws -> EIP681Code? {
            ...
            for comp in queryItems {
                if let inputType = try? ABITypeParser.parseTypeString(comp.name) {
                    guard let rawValue = comp.value,
                          let functionArgument = try await parseFunctionArgument(
                            inputType,
            ...
            switch inputType {
                case .address:
                    ...
                    switch val {
                        ...
                        case .ensAddress(let ens):
                            ...
                            do {
                                guard let network = Network.fromInt(UInt(chainID)) else { return nil }
                                let provider = try await InfuraProvider(net: network)
                                let web = Web3(provider: provider)
                                do {
                                    let registryAddress = try ENS.Registry.registryAddress(for: chainID)
                                    let registry = try ENS.Registry(contractAddress: registryAddress, chainId: chainID)
                                    let resolver = try await registry.getResolver(forDomain: ens, using: web)
                                    let address = try await resolver.getAddress(forNode: ens, using: web)
                                    nativeValue = address
                                } catch {
                ...
                case let .array(type, length):
                    ...
                    for value in rawValues {
                        let intermediateValue = try await parseFunctionArgument(
                            type,
                            value,
                            chainID: chainID,
                            inputNumber: inputNumber
