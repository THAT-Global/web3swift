...
// A tiny box to hold the libsecp256k1 context.
// We know we never mutate or destroy it, so treating it as "effectively immutable and process-wide" is safe in practice.
private struct Secp256k1ContextBox: @unchecked Sendable {
    let raw: OpaquePointer?
    
    init() {
        self.raw = secp256k1_context_create(
...
extension SECP256K1 {
    // Single, process-wide context instance
    private static let contextBox = Secp256k1ContextBox()
    
    /// Immutable context accessor used everywhere in this file
    static var context: OpaquePointer? { contextBox.raw }
